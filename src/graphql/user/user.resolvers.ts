...
import {
  ...
  daoInsertBatch,
  daoInsertRolesBatch,
  deleteEntityRoleResolver,
  EntityRoleResolverOptions,
  getEntityRoleCodes,
  isDefined,
  newEntityRoleResolver,
    ...
    updateUser: async (parent, args, context: ApolloResolversContext, info) => {
      ...
      const error = await startEntityManagerTransaction(context.unsecureEntityManager, context.mongoClient, async (transEntityManager) => {
        ...
        if (isDefined(args.input.roles)) {
          ...
          if (!args.input.roles.some((x) => x === RoleCode.User)) throw new HttpError(400, 'User must have User role!');
        ...
        if (isDefined(args.input.avatar)) {
          if (args.input.avatar.operation === UploadOperation.Insert || args.input.avatar.operation === UploadOperation.Delete) tryDeleteFileIfSelfHosted(user.avatarLink);
        ...
        if (isDefined(args.input.banner)) {
          if (args.input.banner.operation === UploadOperation.Insert || args.input.banner.operation === UploadOperation.Delete) tryDeleteFileIfSelfHosted(user.bannerLink);
