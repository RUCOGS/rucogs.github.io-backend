...
export async function daoInsertBatch(options: {
  ...
  if (options.error === undefined) options.error = true;
  ...
  for (const element of options.elements) {
    ...
    if (entityAdded && options.foreachSuccess) await options.foreachSuccess(element, foundEntity.id);
  }
}
