...
import { getHighestRoles, isRoleBelowOrEqual, makePermsCalc, RoleData, RoleType } from '@src/shared/security';
...
export function newEntityRoleResolver(options: EntityRoleResolverOptions) {
  ...
  return async (parent: any, args: any, context: ApolloResolversContext, info: any) => {
    if (!context.securityContext.userId) throw new HttpError(400, 'Only authenticated users can change roles!');
